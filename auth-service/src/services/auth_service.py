...
class AuthService:
    ...
    async def login_with_sso_identity(
        self,
        user,
        ip_address: Optional[str] = None,
        user_agent: Optional[str] = None,
        permission_source_user=None,
    ) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
        """
        SSO id_token 校验通过后，为已存在的本地用户签发会话与平台 JWT。
        permission_source_user：若传入，则 Token 与返回体中的 roles/permissions 沿用该用户（用于与 admin 权限对齐）。
        """
        try:
            self.db.expire_all()

            # 归一化：当我们传入 permission_source_user 时，按“模拟该用户登录”的语义，
            # 不仅继承权限，还要使用其身份字段生成 JWT/会话（sub/username/email/full_name）。
            perm_user = permission_source_user if permission_source_user is not None else user

            if perm_user.status.value != "active":
                return None, f"用户账户状态为{perm_user.status.value}，无法登录"

            perm_user.last_login_at = datetime.utcnow()
            perm_user.last_login_ip = ip_address
            self.db.commit()

            access_token = self.jwt_manager.create_access_token(
                user_id=str(perm_user.id),
                username=perm_user.username,
                email=perm_user.email,
                roles=[str(r.id) for r in perm_user.roles],
            )
            refresh_token = self.jwt_manager.create_refresh_token(user_id=str(perm_user.id))
            ...
            permissions = self.user_repo.get_user_permissions(str(perm_user.id))
            permission_codes = [p.code for p in permissions] if permissions else []
            ...
            logger.error(f"Error SSO login for user: {str(e)}", exc_info=True)
            self.db.rollback()
            return None, f"SSO 登录失败: {str(e)}"
