...
async def lifespan(app: FastAPI):
    ...
    # DEBUG 开发环境：自动补齐一个默认管理员账号，避免首次启动无法登录
    ...
    if settings.DEBUG:
        try:
            ...
            try:
                ...
                if not user_repo.get_by_username(default_username):
                    ...
                    logger.info("Default admin user already exists, skip seeding.")
            finally:
                session.close()
        except Exception as e:
            logger.error(f"Failed to seed default admin user: {str(e)}", exc_info=True)
