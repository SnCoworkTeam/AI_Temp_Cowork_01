...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadUser = useCallback(async () => {
    try {
      ...
      // accessToken 在 SSO 场景通常来自 HttpOnly Cookie，前端无法读取；
      // 这里即便拿不到 accessToken，也仍然尝试通过 /api/users/me（服务端转发）获取用户信息。
      ...
      try {
        ...
        const userInfoPromise = authClient.getCurrentUser(accessToken || undefined);
  ...
  // 自动刷新令牌
  useEffect(() => {
    // SSO 场景下 token 可能是 HttpOnly cookie，前端无法读取；
    // 这里仅在前端可读到 access/refresh 时才启动定时刷新。
    const accessToken = getAccessToken();
  ...
  const value: AuthContextType = {
    user,
    loading,
    // 不依赖前端可读的 access_token（SSO 为 HttpOnly Cookie 时读不到），以用户加载结果为准
