...
const resolveAuthBaseUrl = () => {
  ...
  // 本地开发强制绕开网关代理：如果配置的是 `http://localhost:8080/api/auth`，
  // 直接转发会导致 /api/auth/login 请求体校验异常（Field required: body）。
  // 这里改为直连 auth-service（默认 8003），避免登录失败。
  if (authBase.includes('/api/auth')) {
    return 'http://localhost:8003/auth';
  }
