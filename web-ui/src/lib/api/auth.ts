...
// 强制走 Next.js 的本地代理路由，避免浏览器直接请求 auth-service 时路径/域名不一致。
// 该代理路由在 `src/app/api/users/me/route.ts` 内部完成转发到网关鉴权接口。
const resolveUsersMeUrl = () => '/api/users/me';
...
class AuthClient {
  ...
  async getCurrentUser(accessToken?: string): Promise<User> {
    ...
    // SSO 场景下 access_token 通常是 HttpOnly Cookie，前端无法读取；
    // 此时应依赖 Cookie 鉴权，让 /api/users/me 转发到网关/认证服务完成鉴权。
