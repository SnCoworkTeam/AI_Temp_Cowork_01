...
class GatewayProxy:
    ...
    async def start(self):
        ...
        self.http_client = httpx.AsyncClient(
            ...
            # 由上游/浏览器处理跳转（如 SSO 回调 302）。
            # 若由网关自动 follow，容易在容器内访问到错误的 localhost/内网地址，导致代理失败。
    ...
    async def forward_request(
        ...
        **kwargs
        ...
        """
        # 取决于“网关运行位置”：
        # - 网关在宿主机上运行：本地访问用 localhost
        # - 网关在容器里运行：访问宿主机端口用 host.docker.internal
        from pathlib import Path
        in_container = Path("/.dockerenv").exists()
        host_for_local = "host.docker.internal" if in_container else "localhost"

        local_service_fallback = {
            "config-center": f"http://{host_for_local}:8090",
            "workflow-engine": f"http://{host_for_local}:8002",
            "auth-service": f"http://{host_for_local}:8003",
            "mcp-gateway": f"http://{host_for_local}:8001",
            "knowledge-base": f"http://{host_for_local}:8004",
            "metadata-service": f"http://{host_for_local}:8005",
            "chat-service": f"http://{host_for_local}:8006",
            "joyagent-adapter": f"http://{host_for_local}:8007",
            "dag-orchestrator": f"http://{host_for_local}:8009",
            "agent-service": f"http://{host_for_local}:8010",
            "agent-orchestrator": f"http://{host_for_local}:8011",
            "agent-registry": f"http://{host_for_local}:8012",
            "project-management": f"http://{host_for_local}:8016",
            "ai-scenario-service": f"http://{host_for_local}:8022",
        }
        docker_service_fallback = {
        ...
        if not endpoint:
            ...
            if use_localhost:
                # 本地开发模式：优先使用 localhost，失败后降级到 docker 服务名
                service_fallback = local_service_fallback
                alternate_fallback = docker_service_fallback
                mode = "local"
            else:
                # Docker 模式：优先使用 docker 服务名，失败后降级到 localhost
                service_fallback = docker_service_fallback
                alternate_fallback = local_service_fallback
                mode = "docker"
            
            endpoint = service_fallback.get(service_name)
            alternate_endpoint = alternate_fallback.get(service_name)
            if endpoint:
        ...
        # 保留客户端原始 Host，供下游构造回跳地址（例如 SSO 回调重定向）。
        if "host" in headers:
            headers["X-Forwarded-Host"] = headers.get("host")
        headers.pop("host", None)  # 移除原始host头
        # 供下游（如 auth-service SSO）判断是否为 HTTPS，以设置 Secure Cookie；外置 TLS 反代请原样转发 X-Forwarded-Proto
        if not any(k.lower() == "x-forwarded-proto" for k in headers) and getattr(
            request.url, "scheme", ""
        ) == "https":
        ...
        for attempt in range(settings.MAX_RETRIES + 1):
            try:
                ...
                # 常见的 DNS 解析失败：自动切换到备用 fallback 地址并重试
                error_text = str(e).lower()
                is_dns_error = ("name or service not known" in error_text) or ("getaddrinfo failed" in error_text)
                if is_dns_error and alternate_endpoint and endpoint != alternate_endpoint:
                    logger.warning(
                        f"DNS lookup failed for {service_name} endpoint {endpoint}, "
                        f"switching to alternate endpoint: {alternate_endpoint}"
                    )
